package com.sasy.nontag.utils.pinlock
 ...
 */
class PinButtonAdapter(val context: Context) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        if (view != null) {
            ...
            if (text == "-2") {
                ...
                button.setOnClickListener {
                    (context as AppPinCodeActivity).onDeleteButtonClicked()
                }
            } else {
                button.layoutParams = LinearLayout.LayoutParams(buttonSize, buttonSize)
                button.text = text
                button.setOnClickListener {
                    (context as AppPinCodeActivity).onPinButtonClicked(text)
                }
            }
        }
